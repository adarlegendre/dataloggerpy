                    ...
                    while not stop_event.is_set():
                        if ser.in_waiting:
                            try:
                                ...
                                if data.startswith(b'\r*'):  # Check for \r* prefix
                                    try:
                                        # Decode bytes to string, remove \r and \n, and strip whitespace
                                ...
                                elif data.strip():  # Only process non-empty data
                                    # Handle non-standard format
                                    logger.warning(f"Received non-standard format from radar {radar.id}: {data}")
                                    data_str = data.decode('utf-8', errors='replace').replace('\r', '').replace('\n', '').strip()
                                    if data_str:  # Only create data_dict if we have actual data
                                        data_dict = {
                                            'status': 'success',
                                            'raw_data': data_str,
                                            'timestamp': time.time(),
                                            'connection_status': 'connected',
                                            'display_text': f'[CONNECTED] {data_str}'  # Add display text for terminal
                                        }
